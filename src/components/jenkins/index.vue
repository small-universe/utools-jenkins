  ...
  return [
    {
      ...
      render(row: any) {
        return h(StatusIcon, {
          ...
          onClick: () => {
            if (
              row._class === JenkinsClassTypeEnum.WORKFLOW_MULTI_BRANCH_PROJECT
            ) {
        ...
        if (row.buildStatus && row.buildStatus !== "FINISH") {
          ...
          ops[0] = h(
            NButton,
            {
              ...
              disabled:
                row._class ===
                JenkinsClassTypeEnum.WORKFLOW_MULTI_BRANCH_PROJECT,
...
export default defineComponent({
  ...
  setup() {
    ...
    const jobParameterTypeOfSelectNoChoice = ref([
      "PT_MULTI_SELECT",
      "PT_SINGLE_SELECT",
      "PT_TAG",
      "PT_BRANCH",
    ...
    const buildJobParamsDialog = async (job: any, index: number) => {
      ...
      try {
        ...
        const data = await store
          .dispatch("getJobAct", job.fullName)
          .then((res) => res.data);
        ...
        if (job.property && job.property.length > 0) {
          for (let property of job.property) {
            if (
              ...
              property.parameterDefinitions.length === 0
            ) {
              continue;
            }
            for (let param of property.parameterDefinitions) {
              job.form[param.name] = param.defaultParameterValue
                ? param.defaultParameterValue.value
                : "";
              await store.dispatch("handleGitParameterAct", {
                job: job,
                param: param,
              });
    ...
    const startBuildJob = () => {
      store
        .dispatch("buildJobAct", {
          jobName: currentBuildJob.value.fullName,
          parameters: currentBuildJob.value.form,
        })
        .then((res) => {
    ...
    const cancelBuildJob = (job: any) => {
      dialog.info({
        ...
        onPositiveClick: () => {
          if (job.curBuildingNumber) {
            store
              .dispatch("cancelBuildAct", {
                jobName: job.fullName,
                num: job.curBuildingNumber,
              })
              .then((res) => {
            ...
            store
              .dispatch("cancelQueueItemAct", job.curBuildingQueueItemId)
              .then((res) => {
    ...
    const showLog = async (job: any) => {
      ...
      await getLog(job).then(() =>
        setTimeout(() => (logLoading.value = false), 1000)
      );
    };
    ...
    const getLog = async (job: any) => {
      return new Promise((resolve, reject) => {
        if (
          job.buildStatus &&
          (job.buildStatus === "SUBMIT" || job.buildStatus === "BUILDING")
        ) {
          timingGetBuildConsole(job);
        } else {
          store
            .dispatch("buildConsoleAct", {
              jobName: job.fullName,
              number: "lastBuild",
              start: 0,
            })
            ...
            .catch((err) => {
              console.error(
                `[getLog] ==> param ==> ${job} fail reason => ${err}`
              );
    ...
    const timingGetBuildConsole = async (job: any) => {
      ...
      if (!job.curBuildingNumber) {
        job.buildConsoleTask = setTimeout(
          () => timingGetBuildConsole(job),
          5000
        );
        return true;
    ...
    const setProgress = (job: any) => {
      ...
      let row = document
        .querySelectorAll(
          ".n-data-table-table .n-data-table-tbody .n-data-table-tr > td:nth-child(2)"
        )
        .item(job.index);
      if (job.buildStatus === "FINISH") {
        ...
        row.setAttribute(
          "style",
          "background: linear-gradient(to right, #D9ECFF " +
            job.progress +
            "%,#ffffff 0%)"
        );
      }
    ...
    const getBuildProgress = async (job: any) => {
      if (job.curBuildingNumber) {
        ...
        store.dispatch("buildHistoryAct", params).then((res) => {
          ...
          if (result.building === false && result.result) {
            ...
            if (estimatedDuration && estimatedDuration !== -1) {
              progress = Math.ceil(
                ((nowTime - startTime) / estimatedDuration) * 100
              );
              if (progress >= 100) {
                progress = 99;
              }
        ...
        await store
          .dispatch("queueItemAct", job.curBuildingQueueItemId)
          .then((res) => {
            let result = res;
      ...
      try {
        ...
        if (!timing) {
          let data = await store
            .dispatch("jobDetailsAct", job.name)
            .then((res) => res.data);
    ...
    const getJobLastBuild = async (job: any) => {
      ...
      try {
        ...
        console.error(
          `[getJobLastBuild] param ==> ${job} ==> fail reason ==> ${error}`
        );
      }
    };
    ...
    const getJobList = (viewName: string, workflowName: string) => {
      store
        .dispatch("jobsAct", { viewName, workflowName })
        .then(async (res) => {
          ...
          for (let job of jobList) {
            ...
            // 设置面包屑数据
            if (!!viewName) {
              breadCrumb.value.view = {
                name: viewName,
                displayName: viewName === "all" ? "所有" : viewName,
              };
            }
            if (!workflowName) {
    ...
    const filterJobList = computed(() => {
      let filter = jobs.value.filter((e: any) =>
        e?.name?.toLowerCase().match(jobName.value?.toLowerCase())
      );
    ...
    const paramsSelectOptions = (params: any) => {
      ...
      if (
        ...
        currentBuildJob.value.parameterProcessed &&
        currentBuildJob.value.parameterProcessed.length > 0
      ) {
        if (
          jobParameterTypeOfSelectNoChoice.value.indexOf(params.type) !== -1
        ) {
    ...
    return {
      ...
      columns: createColumns(
        { buildJobParamsDialog },
        { cancelBuildJob },
        { showLog },
        { handleJobName },
        { handleWorkflowJobIcon },
        { getJobLastBuild }
      ),
      handleJKNameValue,
      handleViewNameValue,
...
<template>
  ...
  <div class="bread-crumb">
    <n-breadcrumb>
      <n-breadcrumb-item
        ...
        @click="handleBreadCrumb(breadCrumb.view.name)"
      >
        <n-icon color="#0e7a0d" size="18">
          <home />
        </n-icon>
        <span style="font-size: 18px; margin: 0px 5px"
          >{{ breadCrumb.view.displayName }}
        </span>
      </n-breadcrumb-item>
      <n-breadcrumb-item v-if="!!breadCrumb.workflow">
        ...
        <span style="font-size: 18px"
          >{{ breadCrumb.workflow.displayName }}
        </span>
      </n-breadcrumb-item>
    </n-breadcrumb>
  </div>
  ...
  <!-- 获取构建参数弹出框 -->
  <n-modal
    v-model:show="showbuildParamsModal"
    preset="dialog"
    title="Dialog"
    :close-on-esc="false"
    :mask-closable="true"
    ...
    <n-space vertical>
      <n-spin :show="buildParamsLoading">
        <n-form-item label="描述" v-if="currentBuildJob.description">
          <label style="color: #a19f9d">{{
            currentBuildJob.description
          }}</label>
        </n-form-item>
        <n-form
          ref="buildJobRef"
          :model="currentBuildJob"
          v-if="currentBuildJob.parameterProcessed"
        >
          <template
            v-for="params in currentBuildJob.parameterProcessed"
            v-bind:key="params.name"
          >
            <n-form-item>
              <template #label>
                {{ params.name }}&nbsp;
              ...
              <n-input
                v-if="jobParameterTypeOfInput.indexOf(params.type) !== -1"
                :type="
                  params.type === 'PasswordParameterDefinition'
                    ? 'password'
                    : 'text'
                "
              ...
              <n-input
                v-else-if="
                  jobParameterTypeOfTextArea.indexOf(params.type) !== -1
                "
                type="textarea"
                v-model:value="currentBuildJob.form[params.name]"
              />
              <n-select
                v-else-if="
                  jobParameterTypeOfSelect.indexOf(params.type) !== -1 &&
                  params.choices &&
                  params.choices.length !== 0
                "
                v-model:value="currentBuildJob.form[params.name]"
                :multiple="params.type === 'PT_MULTI_SELECT'"
                :options="paramsSelectOptions(params)"
                filterable
              />
              <n-switch
                v-else-if="
                  jobParameterTypeOfCheckbox.indexOf(params.type) !== -1
                "
                v-model:value="currentBuildJob.form[params.name]"
              />
        ...
        <template #description> 加载构建参数...... </template>
      </n-spin>
    </n-space>
    <template #action>
  ...
  >
    ...
    <n-space vertical>
      <n-spin :show="logLoading">
        ...
        <template #description> 加载日志...... </template>
      </n-spin>
    </n-space>
  </n-modal>
</template>
